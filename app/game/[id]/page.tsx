    ...
    return (
        <div className="min-w-[1440px] flex flex-col items-center justify-center gap-[33px] p-24">
            ...
            <div className="w-full h-fit items-center justify-center flex flex-col">
                            ...
                            {currentQuiz.options.map((option, index) => (
                                <button key={index} className={`text-4xl text-white p-24  ${currentQuiz.type == 'boolean' ? "bg-[#35CD4D]" : "bg-[#FBAE17]"} rounded-[36px]`} onClick={
                                    () => {
                                        ...
                                        if (selectedOption || option === currentQuiz.answer) {
                                            router.push(`/game/${currentQuiz.id - 1}?correctOrWrong=true`);
                                        } else {
                                            router.push(`/game/${currentQuiz.id - 1}?correctOrWrong=false`);
                                        }
                                    }
                                }>{option}</button>
                            ))}
                ...
                {!view && <Link href={`/game/${Number(params.id) + 1}?view=true`} className="mt-10 p-3 text-2xl bg-green-500 rounded-xl text-white">Next</Link>}
            </div>
        </div>
    );
}
